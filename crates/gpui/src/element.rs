...
pub trait Element: 'static + IntoElement {
    ...
    /// Computes a hash of the element's content for change detection.
    /// If this hash differs from the previous frame, the element is considered dirty.
    /// Default returns 0 which means always-dirty.
    fn content_hash(&self) -> u64 {
        0
    }

    /// Computes a hash of layout-affecting properties.
    /// Default returns 0 which means layout is always recomputed.
    fn layout_hash(&self) -> u64 {
        0
    }

    /// Computes a hash of paint-affecting properties.
    /// Default returns 0 which means paint is always recomputed.
    ...
    E::RequestLayoutState: 'static,
{
    ...
    fn content_hash(&self) -> u64 {
        self.element.content_hash()
    }

    fn layout_hash(&self) -> u64 {
        self.element.layout_hash()
    }

    fn paint_hash(&self) -> u64 {
        self.element.paint_hash()
    }
...
impl AnyElement {
    ...
    pub(crate) fn content_hash(&self) -> u64 {
        ...
        self.inner.content_hash().hash(&mut hasher);
        self.modifiers.hash(&mut hasher);
        hasher.finish()
    }

    pub(crate) fn layout_hash(&self) -> u64 {
        // Layout hash doesn't include modifiers since deferred_priority doesn't affect layout
        self.inner.layout_hash()
    }

    pub(crate) fn paint_hash(&self) -> u64 {
        use std::hash::{Hash, Hasher};
        let mut hasher = std::collections::hash_map::DefaultHasher::new();
        self.inner.paint_hash().hash(&mut hasher);
        self.modifiers.deferred_priority.hash(&mut hasher);
        hasher.finish()
    }
    ...
    pub fn prepaint_at(
        ...
        crate::window::context::PrepaintCx::new(window)
            .with_absolute_element_offset(origin, |window| self.prepaint(window, cx))
    }
