...
use crate::{
    AnyElement, App, Axis, Bounds, Corner, Display, Edges, Element, ElementHashing,
...
impl Element for Anchored {
    ...
    fn content_hash(&self) -> u64 {
        <Self as ElementHashing>::hash_values(|hasher| {
            std::mem::discriminant(&self.anchor_corner).hash(hasher);
            std::mem::discriminant(&self.fit_mode).hash(hasher);
            self.anchor_position
                .map(|p| (p.x.0.to_bits(), p.y.0.to_bits()))
                .hash(hasher);
            std::mem::discriminant(&self.position_mode).hash(hasher);
            self.offset
                .map(|p| (p.x.0.to_bits(), p.y.0.to_bits()))
                .hash(hasher);
    ...
    fn update_render_node(
        ...
        if let Some(anchored_node) = node.as_any_mut().downcast_mut::<AnchoredNode>() {
            anchored_node.update_from(self.config());
...
impl ElementHashing for Anchored {}
...
impl crate::RenderNode for AnchoredNode {
    ...
    fn prepaint_begin(&mut self, ctx: &mut crate::PrepaintCtx) -> crate::PrepaintFrame {
        if let Some(offset) = self.compute_offset(ctx.bounds, &ctx.child_bounds, ctx.window) {
            let abs_offset =
                crate::window::context::PrepaintCx::new(ctx.window).element_offset() + offset;
