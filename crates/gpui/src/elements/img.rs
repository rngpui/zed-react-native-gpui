use crate::{
    ...
    Element, ElementHashing, ElementId, Entity, GlobalElementId, Image, ImageCache,
...
impl Element for Img {
    ...
    fn content_hash(&self) -> u64 {
        <Self as ElementHashing>::hash_values(|hasher| {
            self.source.hash_identity(hasher);
            self.style.grayscale.hash(hasher);
            std::mem::discriminant(&self.style.object_fit).hash(hasher);
...
impl ImageSource {
    /// Hash the identity of the image source for fiber reconciliation.
    /// For Resource variants, hashes the resource itself.
    /// For Arc-wrapped variants, uses pointer identity.
    pub(crate) fn hash_identity<H: Hasher>(&self, state: &mut H) {
        std::mem::discriminant(self).hash(state);
        match self {
            ImageSource::Resource(resource) => resource.hash(state),
            ImageSource::Render(arc) => Arc::as_ptr(arc).hash(state),
            ImageSource::Image(arc) => Arc::as_ptr(arc).hash(state),
            ImageSource::Custom(arc) => Arc::as_ptr(arc).hash(state),
        }
    }
