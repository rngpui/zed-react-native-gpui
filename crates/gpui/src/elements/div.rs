...
use crate::{
    ...
    DispatchPhase, Display, Element, ElementHashing, ElementId, Entity, FocusHandle, Global,
...
impl Element for Div {
    ...
    fn content_hash(&self) -> u64 {
        <Self as ElementHashing>::hash_values(|hasher| {
            use std::hash::Hash;
            self.interactivity.element_id.hash(hasher);
            self.children.len().hash(hasher);
            self.interactivity.base_style.layout_hash().hash(hasher);
            self.interactivity.base_style.paint_hash().hash(hasher);
            self.interactivity.hover_style.is_some().hash(hasher);
            self.interactivity.focus_style.is_some().hash(hasher);
            self.interactivity.active_style.is_some().hash(hasher);
            self.interactivity.focusable.hash(hasher);
            self.interactivity.group.hash(hasher);
        })
    }

    fn layout_hash(&self) -> u64 {
        <Self as ElementHashing>::hash_interactivity_layout(&self.interactivity)
    }

    fn paint_hash(&self) -> u64 {
        <Self as ElementHashing>::hash_interactivity_paint(&self.interactivity)
    }
...
impl crate::RenderNode for DivNode {
    ...
    fn prepaint_begin(&mut self, ctx: &mut crate::PrepaintCtx) -> crate::PrepaintFrame {
        ...
        {
            let intersected = mask.intersect(&PrepaintCx::new(ctx.window).content_mask());
            ctx.window.content_mask_stack.push(intersected);
            frame.pushed_content_mask = true;
        }

        // Push scroll offset
        if !prepaint.scroll_offset.is_zero() {
            let abs_offset = PrepaintCx::new(ctx.window).element_offset() + prepaint.scroll_offset;
            ctx.window.element_offset_stack.push(abs_offset);
            frame.pushed_element_offset = true;
        }
    ...
    fn prepaint_end(&mut self, ctx: &mut crate::PrepaintCtx, frame: crate::PrepaintFrame) {
        ...
        if frame.pushed_element_offset {
            ctx.window.element_offset_stack.pop();
        }
    ...
    fn paint_begin(&mut self, ctx: &mut crate::PaintCtx) -> crate::PaintFrame {
        ...
        log::debug!(
            "[DIVNODE_PAINT_BEGIN] fiber_id={:?}, bounds={:?}",
            ctx.fiber_id,
            ctx.bounds
        );
        ...
        log::debug!(
            "[DIVNODE_PAINT_BEGIN] fiber_id={:?}, hitbox={:?}",
            ctx.fiber_id,
            hitbox.as_ref().map(|h| (h.id, h.bounds))
        );
        ...
        let Some(paint) = self.interactivity.prepare_paint(
            ...
            log::debug!(
                "[DIVNODE_PAINT_BEGIN] fiber_id={:?}, prepare_paint returned None",
                ctx.fiber_id
            );
            return frame;
        };
        ...
        if let Some(mask) = style.overflow_mask(ctx.bounds, ctx.window.rem_size()) {
            let intersected = mask.intersect(&PaintCx::new(ctx.window).content_mask());
            ctx.window.content_mask_stack.push(intersected);
            frame.pushed_content_mask = true;
        }
        ...
        if let Some(scroll_offset) = self.interactivity.scroll_offset.as_ref() {
            // Push scroll offset for paint
            let scroll_offset = *scroll_offset.borrow();
            if !scroll_offset.is_zero() {
                let abs_offset = PaintCx::new(ctx.window).element_offset() + scroll_offset;
                ctx.window.element_offset_stack.push(abs_offset);
                frame.pushed_element_offset = true;
            }
        }
