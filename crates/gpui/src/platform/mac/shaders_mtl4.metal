//! Metal 4 shaders using traditional buffer/texture binding syntax.
//!
//! This file provides the same rendering functionality as shaders.metal but is
//! designed for Metal 4's argument table binding approach. Resources are bound
//! via MTL4ArgumentTable::setAddress:atIndex: on the CPU side.
//!
//! Key design:
//! - Shaders use standard [[buffer(N)]] and [[texture(N)]] attributes
//! - CPU binds GPU addresses to argument table indices matching these bindings
//! - This approach matches Apple's Metal 4 sample code patterns
...
// ============================================================================
// Buffer Binding Indices (shared with Rust code)
// ============================================================================
...
typedef enum TextureBindingIndex : uint32_t {
    TextureBindingIndexAtlas = 0,
    TextureBindingIndexBackdrop = 1,
    TextureBindingIndexIntermediate = 2,
    TextureBindingIndexSurfaceY = 3,
    TextureBindingIndexSurfaceCbCr = 4,
} TextureBindingIndex;

// ============================================================================
// Type definitions come from scene.h (generated by build.rs)
// Forward Declarations for helper functions (defined in shaders.metal)
// ============================================================================
...
// ============================================================================
// Vertex/Fragment Output Structures
// ============================================================================
...
vertex QuadVertexOutput quad_vertex_mtl4(
...
fragment float4 quad_fragment_mtl4(
    ...
    if (corner_center_to_point.x <= 0.0 || corner_center_to_point.y <= 0.0) {
        float2 straight_inner = corner_to_point + reduced_border;
        inner_sdf = -max(straight_inner.x, straight_inner.y);
    } else {
        ...
        if (is_beyond_inner_straight_border) {
            inner_sdf = -1.0;
    ...
    if (border_sdf < antialias_threshold) {
        float4 border_color_val = input.border_color;
        float4 blended_border = over(background_color, border_color_val);
...
vertex ShadowVertexOutput shadow_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
...
vertex UnderlineVertexOutput underline_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
...
// ============================================================================
// Backdrop Blur Shaders
// ============================================================================

vertex BackdropBlurVertexOutput backdrop_blur_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
...
// ============================================================================
// Monochrome Sprite Shaders
// ============================================================================

vertex MonochromeSpriteVertexOutput monochrome_sprite_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
...
fragment float4 monochrome_sprite_fragment_mtl4(
...
// ============================================================================
// Polychrome Sprite Shaders
// ============================================================================

vertex PolychromeSpriteVertexOutput polychrome_sprite_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
    uint sprite_id [[instance_id]],
    constant float2* unit_vertices [[buffer(BufferBindingIndexUnitVertices)]],
    constant Size_DevicePixels* viewport_size [[buffer(BufferBindingIndexViewportSize)]],
...
// ============================================================================
// Path Rasterization Shaders
// ============================================================================

vertex PathRasterizationVertexOutput path_rasterization_vertex_mtl4(
...
vertex PathSpriteVertexOutput path_sprite_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
    uint sprite_id [[instance_id]],
    constant float2* unit_vertices [[buffer(BufferBindingIndexUnitVertices)]],
    constant Size_DevicePixels* viewport_size [[buffer(BufferBindingIndexViewportSize)]],
...
fragment float4 path_sprite_fragment_mtl4(
    PathSpriteVertexOutput input [[stage_in]],
    texture2d<float> intermediate_texture [[texture(TextureBindingIndexIntermediate)]]
) {
...
// ============================================================================
// Surface (YCbCr) Shaders
// ============================================================================

vertex SurfaceVertexOutput surface_vertex_mtl4(
    uint unit_vertex_id [[vertex_id]],
...
fragment float4 surface_fragment_mtl4(
    SurfaceFragmentInput input [[stage_in]],
    texture2d<float> y_texture [[texture(TextureBindingIndexSurfaceY)]],
    texture2d<float> cb_cr_texture [[texture(TextureBindingIndexSurfaceCbCr)]]
) {
