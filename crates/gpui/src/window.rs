...
fn snapshot_hitboxes_into_map(
    ...
    map: &mut FxHashMap<HitboxId, Hitbox>,
) {
    map.clear();
    ...
    while let Some(fiber_id) = stack.pop() {
        ...
        let mut content_mask = data.content_mask.clone();
        if data.reference_origin.is_some() {
            if let Some(current_origin) = fiber_tree
                .bounds
                .get(fiber_id.into())
...
fn resolve_hitbox_for_hit_test(window: &Window, fiber_id: &GlobalElementId) -> Option<Hitbox> {
    let data = window
        .fiber
        .tree
        .hitbox_state
...
pub(crate) struct Frame {
    ...
    pub(crate) hitboxes: FxHashMap<HitboxId, Hitbox>,
...
pub struct FrameDiagnostics {
    ...
    pub quads: usize,
    /// Total backdrop blur primitives in the scene.
...
impl Window {
    ...
    pub(crate) fn resolve_hitbox(&self, fiber_id: &GlobalElementId) -> Option<Hitbox> {
        ...
        let mut content_mask = data.content_mask.clone();
        if data.reference_origin.is_some() {
            if let Some(current_origin) = self
                .fiber
                .tree
                .bounds
                .get((*fiber_id).into())
    ...
    pub(crate) fn resolve_hitbox_for_event(&self, fiber_id: &GlobalElementId) -> Option<Hitbox> {
        resolve_hitbox_for_hit_test(self, fiber_id)
    }
...
impl Window {
    ...
    fn finalize_frame(&mut self, cx: &mut App) {
        ...
        #[cfg(any(debug_assertions, feature = "diagnostics", feature = "test-support"))]
        {
            ...
            self.frame_diagnostics.backdrop_blurs = scene.backdrop_blurs_len(pool);
            ...
            self.frame_diagnostics.estimated_instance_upload_bytes = self
                ...
                .saturating_add(
                    self.frame_diagnostics.backdrop_blurs.saturating_mul(
                        size_of::<crate::scene::BackdropBlur>() + size_of::<TransformationMatrix>(),
                    ),
                )
                .saturating_add(
    ...
    fn reconcile_frame(
        ...
        if !self.refreshing && !viewport_changed && self.fiber.tree.root.is_some() {
            ...
            if self.fiber.tree.get(&root_fiber).is_some() {
                ...
                report.needs_layout = dirty.contains(DirtyFlags::NEEDS_LAYOUT);
                report.needs_paint = dirty.contains(DirtyFlags::NEEDS_PAINT);
            }
            ...
            report.needs_layout =
                viewport_changed
                    || self
                        ...
                        .contains(DirtyFlags::NEEDS_LAYOUT);
            report.needs_paint = self
                .fiber
                .tree
                .dirty_flags(&root_fiber)
                .contains(DirtyFlags::NEEDS_PAINT);
        } else {
            report.needs_layout = viewport_changed;
        }
    ...
    fn draw_roots(&mut self, cx: &mut App) {
        ...
        log::debug!(
            "[FRAME_START] frame={} structure_epoch={}",
            self.fiber.frame_number,
            self.fiber.tree.structure_epoch
        );
        ...
        let layout_fibers = if reconcile_report.needs_layout {
            self.invalidator.set_phase(DrawPhase::Layout);
            TaffyLayoutEngine::setup_taffy_from_fibers(self, root_fiber, cx);
            self.compute_layout_for_fiber(root_fiber, root_size.into(), cx)
        } else {
            0
        };

        if reconcile_report.needs_layout {
    ...
    pub fn paint_path(&mut self, mut path: Path<Pixels>, color: impl Into<Background>) {
        ...
        self.next_frame
            .scene
            .insert_primitive(&mut self.segment_pool, path.scale(scale_factor));
    }
