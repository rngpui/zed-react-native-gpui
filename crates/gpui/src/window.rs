...
use crate::{
    ...
    ImplicitElementId, KeyBinding, KeyContext, KeyDownEvent, KeyEvent, Keystroke, KeystrokeEvent, LayoutId,
...
pub(crate) struct PaintScope {
    ...
    /// The new origin to translate cached primitives to (for replay mode)
    replay_origin: Point<ScaledPixels>,
    /// For capture mode: owned operations being collected
    /// For replay mode: shared reference to cached operations (via Arc)
    capture_operations: SmallVec<[CachedPaintOperation; 8]>,
    replay_operations: Option<std::sync::Arc<SmallVec<[CachedPaintOperation; 8]>>>,
    replay_index: usize,
}
...
impl PaintScope {
    pub(crate) fn capture(origin: Point<ScaledPixels>) -> Self {
        Self {
            ...
            origin,
            replay_origin: Point::default(),
            capture_operations: SmallVec::new(),
    ...
    pub(crate) fn replay(
        operations: std::sync::Arc<SmallVec<[CachedPaintOperation; 8]>>,
        replay_origin: Point<ScaledPixels>,
    ) -> Self {
        Self {
            ...
            replay_origin,
            capture_operations: SmallVec::new(),
            replay_operations: Some(operations),
            replay_index: 0,
        }
    }
    ...
    pub(crate) fn operations_len(&self) -> usize {
        self.replay_operations.as_ref().map_or(0, |ops| ops.len())
    }
}
...
/// A memoized paint result for an element.
///
/// Stores the display items generated by an element's paint() call, keyed by
/// implicit element ID + input hash. If the same element at the same position
/// has the same input hash, we can reuse the display items.
#[derive(Clone)]
pub(crate) struct PaintMemo {
    /// The implicit element ID (position + type based).
    pub implicit_id: ImplicitElementId,
    /// Hash of the element's paint inputs.
    pub input_hash: u64,
    /// Bounds of the element when painted.
    pub bounds: Bounds<Pixels>,
    /// Index range of display items in the DisplayList.
    pub display_item_range: Range<usize>,
}

/// Context for tracking implicit element identity during paint.
///
/// This enables automatic memoization of paint outputs without requiring
/// explicit element IDs. Elements are identified by their position in the tree
/// (parent path + child index + type).
#[derive(Default)]
pub(crate) struct PaintMemoContext {
    /// Stack of parent implicit element IDs.
    implicit_id_stack: SmallVec<[ImplicitElementId; 32]>,
    /// Child counters for each level of the stack.
    child_counters: SmallVec<[u32; 32]>,
    /// Current frame's paint memos (being built).
    current_memos: FxHashMap<u64, PaintMemo>,
    /// Previous frame's paint memos (for cache lookup).
    previous_memos: FxHashMap<u64, PaintMemo>,
}

impl PaintMemoContext {
    /// Create a new paint memo context.
    pub fn new() -> Self {
        let mut ctx = Self::default();
        // Start with root implicit ID
        ctx.implicit_id_stack.push(ImplicitElementId::ROOT);
        ctx.child_counters.push(0);
        ctx
    }

    /// Begin a new frame - swap current and previous memos.
    pub fn begin_frame(&mut self) {
        std::mem::swap(&mut self.current_memos, &mut self.previous_memos);
        self.current_memos.clear();
        self.implicit_id_stack.clear();
        self.implicit_id_stack.push(ImplicitElementId::ROOT);
        self.child_counters.clear();
        self.child_counters.push(0);
    }

    /// Get the current implicit element ID.
    pub fn current_implicit_id(&self) -> ImplicitElementId {
        *self.implicit_id_stack.last().unwrap_or(&ImplicitElementId::ROOT)
    }

    /// Push a new element onto the implicit ID stack.
    /// Returns the implicit ID for this element.
    pub fn push_element<E: 'static>(&mut self) -> ImplicitElementId {
        let parent = self.current_implicit_id();
        let child_index = *self.child_counters.last().unwrap_or(&0);

        // Increment the child counter for the current level
        if let Some(counter) = self.child_counters.last_mut() {
            *counter += 1;
        }

        // Compute the implicit ID for this element
        let implicit_id = parent.child::<E>(child_index);

        // Push onto stack for children
        self.implicit_id_stack.push(implicit_id);
        self.child_counters.push(0);

        implicit_id
    }

    /// Pop the current element from the implicit ID stack.
    pub fn pop_element(&mut self) {
        self.implicit_id_stack.pop();
        self.child_counters.pop();
    }

    /// Check if we have a cached paint memo for this element.
    pub fn get_cached_memo(&self, implicit_id: ImplicitElementId, input_hash: u64, bounds: Bounds<Pixels>) -> Option<&PaintMemo> {
        let key = implicit_id.cache_key();
        self.previous_memos.get(&key).filter(|memo| {
            memo.input_hash == input_hash && memo.bounds == bounds
        })
    }

    /// Store a paint memo for an element.
    pub fn store_memo(&mut self, memo: PaintMemo) {
        let key = memo.implicit_id.cache_key();
        self.current_memos.insert(key, memo);
    }

    /// Get statistics about memo cache hits/misses.
    pub fn stats(&self) -> (usize, usize) {
        (self.current_memos.len(), self.previous_memos.len())
    }
}

/// Cache entry for implicit element ID-based paint memoization.
#[derive(Clone)]
pub(crate) struct ImplicitCacheEntry {
    /// Hash of the element's paint inputs.
    pub input_hash: u64,
    /// Bounds of the element when painted (scaled pixels).
    pub bounds: Bounds<ScaledPixels>,
    /// Cached paint operations (Arc for efficient cloning).
    pub operations: Arc<SmallVec<[CachedPaintOperation; 8]>>,
}

/// Cache for implicit element ID-based paint memoization (Phase 20).
///
/// Unlike the explicit PrimitiveCache which uses GlobalElementId, this cache
/// uses ImplicitElementId (position + type based identity) to enable automatic
/// caching without requiring explicit element IDs.
pub(crate) struct ImplicitPrimitiveCache {
    /// Current frame's cache entries.
    current: FxHashMap<ImplicitElementId, ImplicitCacheEntry>,
    /// Previous frame's cache entries (for lookup).
    previous: FxHashMap<ImplicitElementId, ImplicitCacheEntry>,
    /// Cache statistics
    hits: usize,
    misses: usize,
}

impl ImplicitPrimitiveCache {
    /// Create a new implicit primitive cache.
    pub fn new() -> Self {
        Self {
    ...
    /// Begin a new frame - swap current and previous cache.
    pub fn begin_frame(&mut self) {
    ...
    /// Look up cached paint operations for an implicit element ID.
    pub fn lookup(
        &mut self,
        implicit_id: ImplicitElementId,
        input_hash: u64,
        bounds: Bounds<ScaledPixels>,
    ) -> Option<CacheReplay> {
        if let Some(entry) = self.previous.get(&implicit_id) {
            if entry.input_hash == input_hash && entry.bounds == bounds {
                self.hits += 1;
                return Some(CacheReplay {
    ...
    /// Insert cached paint operations for an implicit element ID.
    pub fn insert(
        &mut self,
        implicit_id: ImplicitElementId,
        input_hash: u64,
        ...
        self.current.insert(implicit_id, ImplicitCacheEntry {
            input_hash,
            bounds,
...
pub struct Window {
    ...
    /// Cache for implicit element ID-based paint memoization (Phase 20).
    /// Maps (implicit_id, input_hash, bounds) -> cached paint operations.
    implicit_primitive_cache: ImplicitPrimitiveCache,
    ...
    /// Context for implicit element identity and paint memoization (Phase 20).
    paint_memo_context: PaintMemoContext,
...
impl Window {
    pub(crate) fn new(
        ...
        Ok(Window {
            ...
            implicit_primitive_cache: ImplicitPrimitiveCache::new(),
            ...
            paint_memo_context: PaintMemoContext::new(),
...
impl Window {
    ...
    pub fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        ...
        self.paint_memo_context.begin_frame();
        self.implicit_primitive_cache.begin_frame();
    ...
    fn draw_roots(&mut self, cx: &mut App) {
        ...
        // Phase 17: Composite root layer's DisplayList to Scene.
        // All primitives now flow through DisplayList; root layer is rasterized directly to Scene.
        self.composite_root_layer();
    }

    /// Composite the root layer's DisplayList to Scene primitives.
    /// This converts DisplayItems to Scene primitives for rendering.
    fn composite_root_layer(&mut self) {
        use crate::LayerId;

        let scale_factor = self.scale_factor;

        // Get root layer and prepare its DisplayList
        let root_layer = self.layer_tree.get_mut(LayerId::ROOT);
    ...
    /// Push a new element onto the implicit ID stack for position-based caching (Phase 20).
    /// Returns the implicit ID for this element.
    pub(crate) fn push_implicit_element<E: 'static>(&mut self) -> ImplicitElementId {
        self.paint_memo_context.push_element::<E>()
    }

    /// Pop the current element from the implicit ID stack (Phase 20).
    pub(crate) fn pop_implicit_element(&mut self) {
        self.paint_memo_context.pop_element()
    }

    /// Look up cached paint operations using implicit element ID (Phase 20).
    pub(crate) fn lookup_implicit_primitive_cache(
        &mut self,
        implicit_id: ImplicitElementId,
        input_hash: u64,
        bounds: Bounds<ScaledPixels>,
    ) -> Option<CacheReplay> {
        self.implicit_primitive_cache.lookup(implicit_id, input_hash, bounds)
    }

    /// Insert cached paint operations using implicit element ID (Phase 20).
    pub(crate) fn insert_implicit_primitive_cache(
        &mut self,
        implicit_id: ImplicitElementId,
        input_hash: u64,
        bounds: Bounds<ScaledPixels>,
        ...
        self.implicit_primitive_cache.insert(implicit_id, input_hash, bounds, operations);
    }
    ...
    fn log_primitive_cache_stats(&mut self) {
        let stats = self.primitive_cache.peek_stats();
    ...
    pub fn take_primitive_cache_stats(&mut self) -> PrimitiveCacheStats {
        ...
        // Update scene stability tracking when stats are taken.
        // This ensures the optimization works even when stats are taken externally (e.g., by benchmarks).
        self.last_frame_was_stable = stats.is_scene_stable();
        stats
    }
    ...
    fn replay_next_primitive(&mut self) -> bool {
        ...
        let Some(operation) = scope.replay_operations.as_ref().and_then(|ops| ops.get(scope.replay_index)).cloned() else {
            return false;
        };
        let replay_origin = scope.replay_origin;
        scope.replay_index += 1;

        match operation {
            CachedPaintOperation::Primitive(primitive) => {
                // replay_origin is the new absolute position; cached primitives have
                // relative coordinates, so we add the origin to get absolute coords.
                // We also apply the CURRENT content_mask for proper clipping after scroll.
                let current_mask = self.content_mask().scale(self.scale_factor());
                CachedPaintOperation::Primitive(primitive)
                    .replay(replay_origin, current_mask, &mut self.next_frame.scene);
                true
            }
            other => {
    ...
    fn insert_primitive_internal(&mut self, primitive: impl Into<Primitive>) {
        ...
        if let Some(scope) = self.paint_scopes.last_mut() {
            if scope.mode == PaintScopeMode::Capture {
                ...
                scope.capture_operations.push(operation);
            }
        }

        // Phase 17: ALL primitives route through the current layer's DisplayList.
        // This includes the root layer, which now has a DisplayList.
        // The DisplayList is later rasterized/composited to the final Scene.
        let scale_factor = self.scale_factor;
        ...
        // Phase 20: Skip primitive insertion if current element has cache hit.
        // The element's own items have already been copied from the previous frame.
        // Children will have their own element entries with their own cache_hit flags.
        ...
        // Primitives no longer go directly to Scene - they're composited later
    }
    ...
    fn pop_layer_internal(&mut self) {
        ...
        if let Some(scope) = self.paint_scopes.last_mut() {
            if scope.mode == PaintScopeMode::Capture {
                scope.capture_operations.push(CachedPaintOperation::EndLayer);
            }
        }
    ...
    ///
    /// Every element gets a `ComputedElementId` that uniquely identifies it:
    /// - If the element has an explicit `global_id` (via `.id()`), uses that
    /// - Otherwise, computes an implicit ID from tree position + type
    ///
    /// This enables automatic caching without requiring explicit element IDs.
    ///
    /// Note: This always pushes onto paint_memo_context to track position
    /// for child elements. The corresponding pop happens in finalize_element_paint().
    pub(crate) fn compute_element_id<E: 'static>(
        &mut self,
        global_id: Option<&GlobalElementId>,
    ) -> crate::display_list::ComputedElementId {
        use std::any::TypeId;

        // Always push onto paint_memo_context to track child positions correctly.
        // Even explicit ID elements need this so their implicit ID children
        // get correct position-based identities.
        let implicit_id = self.paint_memo_context.push_element::<E>();

        if let Some(global) = global_id {
            // Explicit ID: use GlobalElementId-based identity
            crate::display_list::ComputedElementId::from_global(global, TypeId::of::<E>())
        } else {
            // Implicit ID: use position-based identity
            crate::display_list::ComputedElementId::from_implicit(&implicit_id)
        }
    }
    ...
    /// implement paint_inputs_hash()).
    pub(crate) fn begin_element_paint(
        &mut self,
    ...
    pub(crate) fn finalize_element_paint(&mut self) {
        // Pop from paint memo context (to balance push_element in compute_element_id)
        self.paint_memo_context.pop_element();
