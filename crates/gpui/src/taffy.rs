...
impl TaffyLayoutEngine {
            ...
            + 'static,
    {
        ...
        window.fiber.tree.invalidate_layout_cache_upwards(&fiber_id);
        Self::layout_id(&fiber_id)
    }
    ...
    pub(crate) fn compute_layout(
        ...
        fiber_tree.set_layout_context(window as *mut Window, cx as *mut App, scale_factor);
    ...
    pub(crate) fn setup_taffy_from_fibers(
        ...
        window.layout_engine.fibers_layout_changed.clear();
    ...
    pub(crate) fn finalize_dirty_flags(window: &mut Window) {
        ...
        for fiber_id in &window.layout_engine.fibers_layout_changed {
            ...
            if position_changed {
                flags.insert(crate::DirtyFlags::POSITION_CHANGED);
            }
            if size_changed {
                flags.insert(crate::DirtyFlags::SIZE_CHANGED);
            }
            if flags.any() {
        ...
        for (fiber_id, flags) in changed_fibers {
            ...
            if flags
                .intersects(crate::DirtyFlags::POSITION_CHANGED | crate::DirtyFlags::SIZE_CHANGED)
            {
                window.fiber.tree.propagate_position_changed(&fiber_id);
            }
        }
    }
}
...
pub(crate) fn layout_bounds(
    ...
    let layout_state = window
        .fiber
        .tree
        .layout_state
        .get((*fiber_id).into())
        .unwrap_or_else(|| panic!("missing layout state {fiber_id:?}"));
    let final_layout = layout_state.final_layout;
