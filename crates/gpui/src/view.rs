...
impl<V: Render> Element for Entity<V> {
    ...
    fn content_hash(&self) -> u64 {
        ...
        self.entity_id().hash(&mut hasher);
        hasher.finish()
    }
...
impl RenderNode for ViewNode {
    ...
    fn layout_begin(&mut self, ctx: &mut crate::LayoutCtx) -> crate::LayoutFrame {
        ...
        log::debug!(
            "[VIEWNODE] layout_begin: view_id={:?}, fiber_id={:?}, registering in view_roots",
            self.view_id,
            ctx.fiber_id
        );
        ctx.window
    ...
    fn prepaint_begin(&mut self, ctx: &mut crate::PrepaintCtx) -> crate::PrepaintFrame {
        ...
        log::debug!(
            "[VIEWNODE] prepaint_begin: view_id={:?}, fiber_id={:?}",
            self.view_id,
            ctx.fiber_id
        );
    ...
    fn paint_begin(&mut self, ctx: &mut crate::PaintCtx) -> crate::PaintFrame {
        ...
        log::debug!(
            "[VIEWNODE] paint_begin: view_id={:?}, fiber_id={:?}",
            self.view_id,
            ctx.fiber_id
        );
        crate::PaintFrame {
