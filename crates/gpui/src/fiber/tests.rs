    ...
    fn mark_dirty_layout_propagates_to_ancestors() {
        ...
        tree.mark_dirty(&grandchild, DirtyFlags::NEEDS_LAYOUT);

	        let grandchild_dirty = tree.dirty_flags(&grandchild);
	        assert!(grandchild_dirty.contains(DirtyFlags::NEEDS_LAYOUT));

	        let child_dirty = tree.dirty_flags(&child);
	        assert!(child_dirty.contains(DirtyFlags::NEEDS_LAYOUT));
	        assert!(child_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT));
	        assert!(!child_dirty.contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT));
	        assert!(!child_dirty.contains(DirtyFlags::POSITION_CHANGED));

	        let root_dirty = tree.dirty_flags(&root);
	        assert!(root_dirty.contains(DirtyFlags::NEEDS_LAYOUT));
	        assert!(root_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT));
	        assert!(!root_dirty.contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT));
	        assert!(!root_dirty.contains(DirtyFlags::POSITION_CHANGED));
    }

    #[test]
    fn mark_dirty_position_sets_position_descendant_only() {
        let mut tree = FiberTree::new();
        let (root, child, grandchild) = build_chain(&mut tree);
        clear_dirty(&mut tree, &[root, child, grandchild]);

        tree.mark_dirty(&grandchild, DirtyFlags::POSITION_CHANGED);

	        let grandchild_dirty = tree.dirty_flags(&grandchild);
	        assert!(grandchild_dirty.contains(DirtyFlags::POSITION_CHANGED));
	        assert!(!grandchild_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT));

	        let child_dirty = tree.dirty_flags(&child);
	        assert!(!child_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT));
	        assert!(child_dirty.contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT));
	        assert!(!child_dirty.contains(DirtyFlags::NEEDS_LAYOUT));
	        assert!(!child_dirty.contains(DirtyFlags::NEEDS_PAINT));

	        let root_dirty = tree.dirty_flags(&root);
	        assert!(!root_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT));
	        assert!(root_dirty.contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT));
	        assert!(!root_dirty.contains(DirtyFlags::NEEDS_LAYOUT));
	        assert!(!root_dirty.contains(DirtyFlags::NEEDS_PAINT));
    }
    ...
    fn dirty_flags_combine_correctly() {
        ...
        assert!(!flags.contains(DirtyFlags::POSITION_CHANGED));
    }
    ...
    #[test]
    fn can_replay_prepaint_is_blocked_by_position_changes() {
        let mut tree = FiberTree::new();
        ...
        // A position change should invalidate prepaint for the moved fiber and its ancestors.
        tree.mark_dirty(&child, DirtyFlags::POSITION_CHANGED);
        assert!(
            !tree.can_replay_prepaint(&child),
            "position-changed fibers must re-run prepaint so bounds-dependent state updates"
        );
        assert!(
            !tree.can_replay_prepaint(&root),
            "ancestors with position-dirty descendants must not replay prepaint"
        );
    }
