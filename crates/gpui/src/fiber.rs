...
use crate::{
    ...
    SceneSegmentId, SharedString, Size, Style, TabStopMap, TooltipId, VKey, ViewData, Window,
    clear_active_tooltip_if_not_hoverable, handle_tooltip_mouse_move,
};
...
impl DirtyFlags {
    ...
    /// Position changed due to ancestor layout updates
    pub const POSITION_CHANGED: DirtyFlags = DirtyFlags(0b0100);
    /// Size changed due to layout updates
    pub const SIZE_CHANGED: DirtyFlags = DirtyFlags(0b1000);
    /// A descendant has dirty flags set
    pub const HAS_DIRTY_DESCENDANT: DirtyFlags = DirtyFlags(0b0001_0000);
    /// Structure changed (children added/removed/reordered)
    pub const STRUCTURE_CHANGED: DirtyFlags = DirtyFlags(0b0010_0000);
    /// Style changed (layout-affecting or paint-only)
    pub const STYLE_CHANGED: DirtyFlags = DirtyFlags(0b0100_0000);
    /// A descendant has position-only changes
    pub const HAS_POSITION_DIRTY_DESCENDANT: DirtyFlags = DirtyFlags(0b1000_0000);
    /// All flags set (for initial/full render)
    pub const ALL: DirtyFlags = DirtyFlags(0b1111_1111);
    ...
    pub fn needs_work(self) -> bool {
        const WORK_FLAGS: u8 = DirtyFlags::NEEDS_LAYOUT.0
            ...
            | DirtyFlags::POSITION_CHANGED.0
            | DirtyFlags::SIZE_CHANGED.0
    ...
    pub fn needs_prepaint(self) -> bool {
        // Prepaint must run when bounds change so render nodes can update any cached
        // bounds-dependent state (hitboxes, content masks, text bounds, etc.).
        //
        // In particular, `POSITION_CHANGED` must be treated as a prepaint-invalidating flag,
        // otherwise nodes that cache bounds during prepaint (e.g. Text) can paint using stale
        // coordinates after layout changes (such as window resizes).
    ...
    /// Check if this fiber needs paint recomputation (includes position-only changes).
    pub fn needs_paint(self) -> bool {
        const PAINT_FLAGS: u8 = DirtyFlags::NEEDS_PAINT.0
            | DirtyFlags::POSITION_CHANGED.0
            | DirtyFlags::SIZE_CHANGED.0
            | DirtyFlags::STRUCTURE_CHANGED.0
            | DirtyFlags::STYLE_CHANGED.0;
        (self.0 & PAINT_FLAGS) != 0
    }

    /// Check if this subtree is completely clean (no flags at all).
    /// Use this for replay gating - requires ALL flags to be clear.
    /// This is stricter than !needs_work() which ignores HAS_DIRTY_DESCENDANT.
...
impl std::ops::Sub for DirtyFlags {
    type Output = Self;
...
pub struct Fiber {
    ...
    /// Hash of (child keys + kinds) for structure comparison
...
impl Default for FiberViewState {
    fn default() -> Self {
        Self {
            view_data: None,
            legacy_element: None,
            view_descriptor_hash: 0,
        }
    }
}
...
pub(crate) struct FiberHitboxState {
    ...
    pub hitbox_subtree_bounds: Option<Bounds<Pixels>>,
}

impl Default for FiberHitboxState {
    fn default() -> Self {
        Self {
            hitbox: None,
            hitbox_subtree_bounds: None,
        }
    }
}
...
impl Default for FiberPaintCache {
    fn default() -> Self {
        Self {
            prepaint_state: None,
            paint_list: None,
            scene_segment_list: None,
        }
    }
}
...
impl FiberTree {
    ...
    pub fn create_fiber_for(&mut self, descriptor: &AnyElement) -> GlobalElementId {
        ...
        let fiber = Fiber::with_key(key, descriptor_hash, layout_hash, paint_hash, child_count);
    ...
    pub fn create_placeholder_fiber(&mut self) -> GlobalElementId {
        let fiber = Fiber::with_key(VKey::None, 0, 0, 0, 0);
        self.insert_fiber(fiber)
    }
    ...
    pub fn create_child_fiber(
        ...
        for _ in 0..placeholders_needed {
            let placeholder = Fiber::with_key(VKey::None, 0, 0, 0, 0);
    ...
    pub fn ensure_pending_view_fiber(&mut self, view_id: EntityId) -> GlobalElementId {
        ...
        let fiber = Fiber::with_key(VKey::View(view_id), 0, 0, 0, 0);
    ...
    pub(crate) fn set_dirty_flags(&mut self, id: &GlobalElementId, dirty: DirtyFlags) {
        self.dirty.insert(Self::key_for_id(*id), dirty);
    }
    ...
    pub fn mark_dirty(&mut self, fiber_id: &GlobalElementId, flags: DirtyFlags) {
        let mut had_dirty_work = false;
        let invalidate_layout_cache = flags.intersects(DirtyFlags::NEEDS_LAYOUT);
        let slot_key = Self::key_for_id(*fiber_id);
        ...
        // If this node is marked as needing layout, its own cached layout must be cleared.
        // Otherwise, Taffy may replay a stale LayoutOutput even when the node's structure
        // (children added/removed/reordered) or constraints have changed.
        if invalidate_layout_cache {
        ...
        // Single consolidated walk up to ancestors instead of 2-3 separate walks
        let propagate_layout = invalidate_layout_cache;
        let propagate_prepaint_descendant = flags.intersects(
            DirtyFlags::NEEDS_LAYOUT
                | DirtyFlags::NEEDS_PAINT
                | DirtyFlags::SIZE_CHANGED
                | DirtyFlags::STRUCTURE_CHANGED
                | DirtyFlags::STYLE_CHANGED,
        );
        let propagate_position_descendant = flags.intersects(DirtyFlags::POSITION_CHANGED);
        let mut current = self.parent(fiber_id);

        while let Some(parent_id) = current {
            let parent_key = Self::key_for_id(parent_id);
            let Some(parent_dirty) = self.dirty.get(parent_key).copied() else {
                break;
            };

            let had_dirty_descendant = parent_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT);
            let had_position_descendant =
                parent_dirty.contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT);
            let had_needs_layout = parent_dirty.contains(DirtyFlags::NEEDS_LAYOUT);

            let mut updated_parent_dirty = parent_dirty;

            // Set HAS_DIRTY_DESCENDANT if not already set
            if propagate_prepaint_descendant && !had_dirty_descendant {
                updated_parent_dirty.insert(DirtyFlags::HAS_DIRTY_DESCENDANT);
            }

            if propagate_position_descendant && !had_position_descendant {
                updated_parent_dirty.insert(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT);
            }

            // Propagate NEEDS_LAYOUT and clear taffy cache if needed
            if propagate_layout {
                if !had_needs_layout {
                    updated_parent_dirty.insert(DirtyFlags::NEEDS_LAYOUT);
                }
                if let Some(layout_state) = self.layout_state.get_mut(parent_key) {
                    layout_state.taffy_cache.clear();
                }
            }

            if updated_parent_dirty != parent_dirty {
                self.set_dirty_flags(&parent_id, updated_parent_dirty);
            }

            // Stop early if all required flags were already set (ancestors already propagated)
            if (!propagate_prepaint_descendant || had_dirty_descendant)
                && (!propagate_position_descendant || had_position_descendant)
                && (!propagate_layout || had_needs_layout)
            {
                break;
            }
    ...
    /// Propagate POSITION_CHANGED flag down to descendants.
    pub fn propagate_position_changed(&mut self, fiber_id: &GlobalElementId) {
        let mut had_dirty_work = false;
        let mut stack: Vec<GlobalElementId> = self.children_vec(fiber_id);
        while let Some(current_id) = stack.pop() {
            let key = Self::key_for_id(current_id);
            if let Some(dirty) = self.dirty.get_mut(key) {
                dirty.insert(DirtyFlags::POSITION_CHANGED);
                had_dirty_work = true;
                stack.extend(self.children(&current_id));
            }
        }
        if had_dirty_work {
            self.had_dirty_work = true;
    ...
    /// Get children as an owned Vec, useful when you need to iterate while mutating
    pub fn children_vec(&self, parent_id: &GlobalElementId) -> Vec<GlobalElementId> {
        self.children_slice(parent_id).to_vec()
    }
    ...
    pub fn can_replay_prepaint(&self, fiber_id: &GlobalElementId) -> bool {
        // Prepaint cannot be replayed if any descendant has position-only changes,
        // because skipping prepaint would skip updating bounds-dependent state.
        self.can_replay_cached(fiber_id, false, true)
    }

    /// Check if a fiber can replay its cached paint list.
    pub fn can_replay_paint(&self, fiber_id: &GlobalElementId) -> bool {
        self.can_replay_cached(fiber_id, true, true)
    }

    fn can_replay_cached(
        &self,
        fiber_id: &GlobalElementId,
        check_paint: bool,
        check_position_descendant: bool,
    ) -> bool {
        if self.get(fiber_id).is_none() {
            return false;
        };
        ...
        !needs_work
            && !dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT)
            && (!check_position_descendant
                || !dirty.contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT))
    ...
    /// End-of-frame cleanup: clears per-frame work flags and recomputes HAS_DIRTY_DESCENDANT.
    ///
    /// This should be called at the end of each frame after all rendering is complete.
    /// It clears the work flags (NEEDS_LAYOUT, NEEDS_PAINT, STYLE_CHANGED, STRUCTURE_CHANGED,
    /// POSITION_CHANGED, SIZE_CHANGED) and properly recomputes HAS_DIRTY_DESCENDANT bottom-up.
    ///
    ...
    pub fn end_of_frame_cleanup(&mut self) {
        ...
        // HAS_DIRTY_DESCENDANT is already accurate, and no work flags need clearing.
        if !self.had_dirty_work {
            return;
        }

        const WORK_FLAGS: DirtyFlags = DirtyFlags(
            DirtyFlags::NEEDS_LAYOUT.0
                | DirtyFlags::NEEDS_PAINT.0
                | DirtyFlags::STYLE_CHANGED.0
                | DirtyFlags::STRUCTURE_CHANGED.0
                | DirtyFlags::POSITION_CHANGED.0
        ...
        // First pass: clear work flags from all fibers and reset HAS_DIRTY_DESCENDANT.
        // Work flags represent "work that needs to be done". After layout/prepaint/paint
        // have completed, these flags should be cleared for ALL fibers, including ones
        // that were processed this frame (e.g., deferred elements processed in deferred pass).
        for (_id, dirty) in self.dirty.iter_mut() {
            dirty.remove(WORK_FLAGS);
            dirty.remove(DirtyFlags::HAS_DIRTY_DESCENDANT);
            dirty.remove(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT);
        }

        // Second pass: recompute HAS_DIRTY_DESCENDANT bottom-up.
        //
        // This must include detached trees (e.g. retained overlay roots for `Window::defer_draw`)
        // so cached-subtree replay remains correct for out-of-tree roots.
        let mut roots: Vec<GlobalElementId> = Vec::new();
        ...
        for root_id in roots {
            ...
            // Build post-order list using iterative DFS.
            while let Some(fiber_id) = stack.pop() {
                if !visited.insert(fiber_id) {
                    continue;
                }
                post_order.push(fiber_id);
                stack.extend(self.children(&fiber_id));
            }

            // Process in reverse (post-order: children before parents).
            for fiber_id in post_order.iter().rev() {
                let (has_prepaint_dirty_child, has_position_dirty_child) = self
                    .get(fiber_id)
                    .map(|_fiber| {
                        let mut prepaint_dirty = false;
                        let mut position_dirty = false;
                        for child_id in self.children_slice(fiber_id) {
                            if self.get(child_id).is_none() {
                                continue;
                            };
                            let child_dirty = self.dirty_flags(child_id);
                            if child_dirty.needs_prepaint()
                                || child_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT)
                            {
                                prepaint_dirty = true;
                            }
                            if child_dirty.contains(DirtyFlags::POSITION_CHANGED)
                                || child_dirty
                                    .contains(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT)
                            {
                                position_dirty = true;
                            }
                            if prepaint_dirty && position_dirty {
                                break;
                            }
                        }
                        (prepaint_dirty, position_dirty)
                    })
                    .unwrap_or((false, false));

                if let Some(dirty) = self.dirty.get_mut(Self::key_for_id(*fiber_id)) {
                    if has_prepaint_dirty_child {
                        dirty.insert(DirtyFlags::HAS_DIRTY_DESCENDANT);
                    }
                    if has_position_dirty_child {
                        dirty.insert(DirtyFlags::HAS_POSITION_DIRTY_DESCENDANT);
                    }
                }
            }
        }
    ...
    pub fn invalidate_layout_cache_upwards(&mut self, fiber_id: &GlobalElementId) {
        let mut current = Some(*fiber_id);
        while let Some(id) = current {
            let Some(layout_state) = self.layout_state.get_mut(Self::key_for_id(id)) else {
                break;
            };
            if layout_state.taffy_cache.is_empty() {
                break;
            }
            layout_state.taffy_cache.clear();
    ...
    pub fn reconcile(
        ...
        let mut view_dirty_flags = DirtyFlags::NONE;
        if let VKey::View(entity_id) = descriptor.key() {
            let mut hasher = FxHasher::default();
            entity_id.hash(&mut hasher);
            let new_view_hash = hasher.finish();
            if let Some(view_state) = self.view_state.get_mut(Self::key_for_id(*fiber_id)) {
                if view_state.view_descriptor_hash != new_view_hash {
                    view_dirty_flags |= DirtyFlags::STRUCTURE_CHANGED;
                    view_dirty_flags |= DirtyFlags::NEEDS_LAYOUT | DirtyFlags::NEEDS_PAINT;
                    view_state.view_descriptor_hash = new_view_hash;
                    view_changed = true;
                }
            }
            if view_dirty_flags != DirtyFlags::NONE {
                self.mark_dirty(fiber_id, view_dirty_flags);
            }
        }
    ...
    fn reconcile_internal(
        ...
        if let Some(fiber) = self.fibers.get_mut(slot_key) {
            let content_unchanged = fiber.descriptor_hash == new_descriptor_hash;
            let layout_unchanged = fiber.layout_hash == new_layout_hash;
            let paint_unchanged = fiber.paint_hash == new_paint_hash;
            let children_unchanged = fiber.child_count == new_child_count;
            let has_dirty_descendant = existing_dirty.contains(DirtyFlags::HAS_DIRTY_DESCENDANT);
            let has_self_work = existing_dirty.needs_work();
            if allow_kind_change {
                let new_key = descriptor.key();
                let is_view_descriptor = matches!(new_key, VKey::View(_));
            ...
            // if they opt into hashing. Hash values of 0 mean "unknown/always dirty".
            //
            // We check render-node support based on the existing fiber state rather than asking
            // the descriptor, since the fiber accurately reflects which path is used at runtime.
            let legacy_has_meaningful_hashes =
                new_descriptor_hash != 0 && new_layout_hash != 0 && new_paint_hash != 0;
            if !has_render_node
                && self
                    .view_state
                    .get(slot_key)
                    .is_some_and(|state| state.legacy_element.is_some())
                && !legacy_has_meaningful_hashes
            {
                dirty_flags |= DirtyFlags::NEEDS_LAYOUT | DirtyFlags::NEEDS_PAINT;
                layout_changed = true;
            }
            ...
            let completely_unchanged =
                content_unchanged && layout_unchanged && paint_unchanged && children_unchanged;
            let no_dirty_work = !has_self_work && !has_dirty_descendant;
            should_bail_out =
                completely_unchanged && no_dirty_work && dirty_flags == DirtyFlags::NONE;
        }
        if allow_bailout && dirty_flags == DirtyFlags::NONE && should_bail_out {
        ...
        if layout_changed {
            let child_ids: Vec<GlobalElementId> = self.children_vec(fiber_id);
            let mut had_dirty_work = false;
            for child_id in child_ids {
                if let Some(dirty) = self.dirty.get_mut(Self::key_for_id(child_id)) {
                    dirty.insert(DirtyFlags::POSITION_CHANGED);
                    had_dirty_work = true;
                }
            }
...
impl TraversePartialTree for FiberTree {
    type ChildIter<'a>
        = std::iter::Copied<std::slice::Iter<'a, GlobalElementId>>
    where
        Self: 'a;

    fn child_ids(&self, node_id: NodeId) -> Self::ChildIter<'_> {
        self.children_slice(&node_id).iter().copied()
    }

    fn child_count(&self, node_id: NodeId) -> usize {
        self.children_slice(&node_id).len()
    }

    fn get_child_id(&self, node_id: NodeId, index: usize) -> NodeId {
        self.children_slice(&node_id)
            .get(index)
            .copied()
...
impl LayoutPartialTree for FiberTree {
    ...
    fn set_unrounded_layout(&mut self, node_id: NodeId, layout: &Layout) {
        if let Some(layout_state) = self.layout_state.get_mut(Self::key_for_id(node_id)) {
            layout_state.unrounded_layout = *layout;
        }
    }
            ...
            match (display_mode, has_children) {
                ...
                (_, false) => {
                    ...
                    >| {
                        ...
                        // Try RenderNode measurement first (new architecture).
                        // The node was taken out of the fiber, so this is safe.
                        ...
                        // Fall back to legacy measure_func (will be removed when all leaf nodes migrate).
...
impl CacheTree for FiberTree {
    fn cache_get(
        ...
        self.layout_state.get(Self::key_for_id(node_id))?.taffy_cache.get(
            known_dimensions,
            available_space,
            run_mode,
        )
...
impl RoundTree for FiberTree {
    ...
    fn set_final_layout(&mut self, node_id: NodeId, layout: &Layout) {
        if let Some(layout_state) = self.layout_state.get_mut(Self::key_for_id(node_id)) {
            layout_state.final_layout = *layout;
        }
    }
}
...
impl FiberRuntimeHandleRef<'_> {
    ...
    pub(super) fn should_render_view_fiber(&self, fiber_id: &GlobalElementId) -> bool {
        if self.window.fiber.tree.get(fiber_id).is_none() {
            log::debug!(
                "[VIEW_RENDER] fiber_id={:?}, decision=false, reason=fiber_not_found",
                fiber_id
            );
            return false;
        };
        ...
        let has_cached_child = self
            ...
            .unwrap_or(false);
        let should_render = !is_clean || !has_cached_child;
        log::debug!(
            "[VIEW_RENDER] fiber_id={:?}, decision={}, is_clean={}, has_cached_output={}, has_prepaint_dirty_ancestor={}, has_cached_child={}",
            fiber_id,
            should_render,
            dirty.is_subtree_clean(),
            has_cached_output,
            has_prepaint_dirty_ancestor,
            has_cached_child
        );
        should_render
    }
...
impl FiberRuntimeHandle<'_> {
    ...
    pub(crate) fn mark_view_dirty(&mut self, view_id: EntityId) {
        log::debug!(
            "[MARK_DIRTY] view_id={:?}, view_roots_count={}",
            view_id,
            self.window.fiber.tree.view_roots.len()
        );

        if let Some(fiber_id) = self.window.fiber.tree.view_roots.get(&view_id).copied() {
            log::debug!(
                "[MARK_DIRTY] Found view fiber_id={:?} for view_id={:?}, marking NEEDS_PAINT",
                fiber_id,
                view_id
            );
            self.window
            ...
            .is_some_and(|root| root.entity_id() == view_id)
        {
            ...
            log::debug!(
                "[MARK_DIRTY] view_id={:?} is root view, fiber_id={:?}, marking NEEDS_PAINT",
                view_id,
                fiber_id
            );
            self.window
        ...
        log::debug!(
            "[MARK_DIRTY] view_id={:?} not found in view_roots and not root, marking root fiber dirty as fallback",
            view_id
        );
        if let Some(root_id) = self.window.fiber.tree.root {
            self.window
                .fiber
                .tree
                .mark_dirty(&root_id, DirtyFlags::NEEDS_PAINT);
        }
    }
    ...
    pub(crate) fn expand_view_fibers(
        ...
        let mut queue: Vec<GlobalElementId> = self
            ...
            .collect();
        log::debug!(
            "[EXPAND_VIEWS] Starting with {} view root fibers in queue",
            queue.len()
        );
        ...
        while let Some(fiber_id) = queue.pop() {
            if !visited.insert(fiber_id) {
                log::debug!(
                    "[EXPAND_VIEWS] fiber_id={:?}, skipped=already_visited",
                    fiber_id
                );
                continue;
            }
            let Some(view_data) = self
                ...
                log::debug!(
                    "[EXPAND_VIEWS] fiber_id={:?}, skipped=no_view_data",
                    fiber_id
                );
                continue;
            };
            ...
            if !self.window.should_render_view_fiber(&fiber_id) {
                log::debug!(
                    "[EXPAND_VIEWS] fiber_id={:?}, skipped=should_not_render",
                    fiber_id
                );
                continue;
            }
            ...
            log::debug!(
                "[EXPAND_VIEWS] fiber_id={:?}, view_id={:?}, rendering_view",
                fiber_id,
                view_data.entity_id()
            );
            ...
            log::debug!(
                "[EXPAND_VIEWS] About to cache_fiber_payloads for fiber_id={:?}, element children count={}",
                fiber_id,
    ...
    fn sync_render_node(
        ...
        let action = if let Some(_fiber) = self.window.fiber.tree.fibers.get_mut(slot_key) {
            ...
            if let Some(priority) = deferred_priority {
                self.window
                    .fiber
                    .tree
                    .deferred_priorities
        ...
        match action {
            RenderNodeAction::TryUpdate => {
                ...
                if let Some(result) = update_result {
                    ...
                    // Mark fiber dirty based on update result (combined into single lookup)
                    let mut dirty_flags = DirtyFlags::NONE;
                    if result.paint_changed {
                        dirty_flags |= DirtyFlags::NEEDS_PAINT;
                    }
                    if result.layout_changed {
                        dirty_flags |= DirtyFlags::NEEDS_LAYOUT;
                    }
                    if dirty_flags != DirtyFlags::NONE {
                        self.window.fiber.tree.mark_dirty(fiber_id, dirty_flags);
                    }
                    true
                } else {
    ...
    pub(crate) fn cache_fiber_payloads(
        ...
        let key = element.key();
        log::debug!(
            "[CACHE_PAYLOAD] Called for fiber_id={:?}, key={:?}, children={}",
            fiber_id,
            key,
            element.children().len()
        );
        ...
        if let crate::VKey::View(entity_id) = key {
            log::debug!(
                "[CACHE_PAYLOAD] Processing View element: entity_id={:?}, fiber_id={:?}",
                entity_id,
                fiber_id
            );
            ...
            log::debug!(
                "[CACHE_PAYLOAD] entity_id={:?}, as_any_view()={}",
                entity_id,
                any_view.is_some()
            );
                ...
                .get_mut((*fiber_id).into())
            {
                ...
                log::debug!(
                    "[CACHE_PAYLOAD] entity_id={:?}, view_data set={}",
                    entity_id,
                    view_state.view_data.is_some()
                );
            ...
            self.window.fiber.tree.set_view_root(entity_id, *fiber_id);
            log::debug!(
                "[CACHE_PAYLOAD] entity_id={:?}, registered in view_roots",
                entity_id
            );
            return;
        }
...
impl StackState {
    fn capture(window: &Window) -> Self {
        Self {
            ...
            offset_len: window.element_offset_stack.len(),
    ...
    fn restore(self, window: &mut Window) {
        ...
        window.element_offset_stack.truncate(self.offset_len);
...
impl FiberRuntimeHandle<'_> {
    ...
    pub(super) fn collect_deferred_draw_keys(&mut self) -> Vec<DeferredDrawKey> {
        ...
        log::debug!(
            "[DEFERRED] Collecting deferred draw keys from {} active fibers",
            self.window.fiber.active_deferred_draws.ordered.len()
        );
            ...
            .copied()
        {
            if let Some(draws) = self.window.fiber.tree.deferred_draws.get(fiber_id.into()) {
                log::debug!(
                    "[DEFERRED] fiber_id={:?}, draw_count={}",
                    fiber_id,
                    draws.len()
                );
                for (index, draw) in draws.iter().enumerate() {
                    log::debug!(
                        "[DEFERRED] fiber_id={:?}, index={}, priority={:?}",
                        fiber_id,
                        index,
                        draw.priority
                    );
        ...
        log::debug!(
            "[DEFERRED] Collected {} total deferred draw keys",
            keys.len()
        );
        keys
    }
    ...
    pub(super) fn prepaint_deferred_draws(
        ...
        log::debug!(
            "[DEFERRED] prepaint_deferred_draws called with {} draws",
            deferred_draws.len()
        );
        for draw_key in deferred_draws {
            log::debug!(
                "[DEFERRED] Processing deferred draw: owner_id={:?}, index={}",
                draw_key.owner_id,
                draw_key.index
            );
            let (
            ...
            if let Some(fiber_id) = fiber_id {
                log::debug!(
                    "[DEFERRED] Processing fiber-backed deferred draw, fiber_id={:?}, requires_layout={}",
                    fiber_id,
                    requires_layout
                );
                self.window.with_rendered_view(current_view, |window| {
                    ...
                    if requires_layout {
                        log::debug!(
                            "[DEFERRED] Deferred draw requires layout for fiber_id={:?}",
                            fiber_id
                        );
                        ...
                        if needs_layout {
                            log::debug!("[DEFERRED] Computing layout for fiber_id={:?}", fiber_id);
                    ...
                    log::debug!(
                        "[DEFERRED] Calling prepaint_fiber_tree_internal for fiber_id={:?}",
                        fiber_id
                    );
                ...
                log::debug!("[DEFERRED] Deferred draw has no fiber_id (element-backed)");
            }
    ...
    fn prepaint_legacy_fiber(&mut self, fiber_id: GlobalElementId, cx: &mut App) {
        log::debug!(
            "[LEGACY_FIBER] prepaint_legacy_fiber called for fiber_id={:?}",
            fiber_id
        );
        let mut legacy = self
            .window
        ...
        log::debug!(
            "[LEGACY_FIBER] Retrieved legacy element: present={}, element_present={}",
            legacy.is_some(),
            legacy.as_ref().and_then(|l| l.element.as_ref()).is_some()
        );
        if let Some(legacy_element) = legacy.as_mut() {
            if let Some(element) = legacy_element.element.as_mut() {
                log::debug!(
                    "[LEGACY_FIBER] Calling prepaint on legacy element type={}",
                    legacy_element.type_name
                );
                ...
                log::debug!(
                    "[LEGACY_FIBER] prepaint completed for fiber_id={:?}",
                    fiber_id
                );
            } else {
                log::warn!(
                    "[LEGACY_FIBER] Legacy element present but element is None for fiber_id={:?}",
                    fiber_id
                );
            }
        } else {
            log::warn!(
                "[LEGACY_FIBER] No legacy element found for fiber_id={:?}",
                fiber_id
            );
        }
    ...
    fn paint_legacy_fiber(&mut self, fiber_id: GlobalElementId, cx: &mut App) {
        log::debug!(
            "[LEGACY_FIBER] paint_legacy_fiber called for fiber_id={:?}",
            fiber_id
        );
        let mut legacy = self
            .window
            ...
            .and_then(|state| state.legacy_element.take());
        log::debug!(
            "[LEGACY_FIBER] Retrieved legacy element for paint: present={}, element_present={}",
            legacy.is_some(),
            legacy.as_ref().and_then(|l| l.element.as_ref()).is_some()
        );
        if let Some(legacy_element) = legacy.as_mut() {
            if let Some(element) = legacy_element.element.as_mut() {
                log::debug!(
                    "[LEGACY_FIBER] Calling paint on legacy element type={}",
                    legacy_element.type_name
                );
                self.window.with_element_id_stack(&fiber_id, |window| {
                    let _guard = GlobalElementIdGuard::new(window, fiber_id);
                ...
                log::debug!("[LEGACY_FIBER] paint completed for fiber_id={:?}", fiber_id);
            } else {
                log::warn!(
                    "[LEGACY_FIBER] Legacy element present but element is None during paint for fiber_id={:?}",
                    fiber_id
                );
            }
        } else {
            log::warn!(
                "[LEGACY_FIBER] No legacy element found for paint of fiber_id={:?}",
                fiber_id
            );
        }
        if let Some(view_state) = self.window.fiber.tree.view_state.get_mut(fiber_id.into()) {
    ...
    pub(crate) fn prepaint_fiber_tree_internal(
        ...
        fn enter_prepaint(
            ...
            // Note: The early skip check at the start of enter_prepaint now handles deferred elements,
            // so this block is unreachable. Kept for reference/future cleanup.
            if false {
                log::debug!("[PREPAINT] Deferring paint for fiber_id={:?}", fiber_id);
                let priority = handle
                    .window
                    .fiber
                    .tree
                ...
                log::debug!(
                    "[PREPAINT] fiber_id={:?}, is_legacy={}, node_handled={}",
                    fiber_id,
                    is_legacy,
                    node_handled
                );
                if node_handled {
                    // Node handled prepaint - use its skip_children value
                    if let Some(ref frame) = render_node_frame {
                        skip_children = frame.skip_children;
                    }
                    log::debug!(
                        "[PREPAINT] Node handled prepaint, skip_children={}",
                        skip_children
                    );
                } else {
                    ...
                    if is_legacy {
                        log::debug!("[PREPAINT] Taking legacy path for fiber_id={:?}", fiber_id);
                        ...
                        // their prepaint called via the fiber pipeline.
                        skip_children = false;
                    } else {
                        log::debug!(
                            "[PREPAINT] Non-legacy fiber with no render node handling, fiber_id={:?}",
                            fiber_id
                        );
                        skip_children = true;
                    }
        ...
        fn exit_prepaint(state: &mut PrepaintTraversal<'_>, fiber_id: GlobalElementId) {
            ...
            let subtree_hitbox_bounds = {
                ...
                for child_id in window.window.fiber.tree.children_slice(&fiber_id) {
                    ...
                    {
                        if child_bounds.size.width > crate::px(0.)
                            && child_bounds.size.height > crate::px(0.)
                        {
                ...
                if let Some((data, reference_origin)) = window
                    .window
                    .fiber
                    .tree
                    .hitbox_state
                    ...
                    .and_then(|state| {
                        state
                            .hitbox
                            .as_ref()
                            .map(|data| (data, data.reference_origin))
                    })
                {
                    let mut bounds = data.bounds;
                    let mut content_mask_bounds = data.content_mask.bounds;

                    if reference_origin.is_some() {
                        if let Some(current_origin) = window
                            .window
                            .fiber
                            .tree
                            .bounds
                        ...
                        {
                            bounds = Bounds {
                                origin: data.local_bounds.origin + current_origin,
                                size: data.local_bounds.size,
                            };
                            content_mask_bounds = Bounds {
                                origin: data.local_content_mask_bounds.origin + current_origin,
                                size: data.local_content_mask_bounds.size,
                            };
                        }
                    }

                    let bounds = bounds.intersect(&content_mask_bounds);
    ...
    pub(crate) fn paint_fiber_tree_internal(
        ...
        fn unwind_paint_frame(window: &mut FiberRuntimeHandle<'_>, frame: &PaintFrame) {
            ...
            if frame.pushed_offset {
                window.window.element_offset_stack.pop();
            }
        ...
        fn enter_paint(
            ...
            let handle = state.ctx.handle_mut();
            let cx = state.ctx.app_mut();
            let (is_legacy, can_replay, should_skip_work, mut paint_scene) = {
                let Some(_fiber) = handle.window.fiber.tree.get(&fiber_id) else {
                    return TraversalDecision::SkipSubtree;
                };
                let is_legacy = handle
                ...
                let mut paint_scene = handle.window.fiber.tree.dirty_flags(&fiber_id).needs_paint()
                    || handle
                        .window
                        .fiber
                        .tree
                        .has_prepaint_dirty_ancestor(&fiber_id);
                (is_legacy, can_replay, should_skip_work, paint_scene)
            };
            ...
            if clipped_bounds.is_empty() {
                if let Some(segment_list) = handle
                    .window
                    .fiber
                    .tree
                    .paint_cache
        ...
        fn exit_paint(state: &mut PaintTraversal<'_>, fiber_id: GlobalElementId) {
            ...
            if let Some(hitbox) = frame.hitbox.as_ref() {
                #[cfg(any(feature = "inspector", debug_assertions))]
                window.window.insert_inspector_hitbox(hitbox.id, None, cx);
    ...
    pub(crate) fn defer_fiber_draw(
        ...
        log::debug!(
            "[DEFERRED] defer_fiber_draw called for fiber_id={:?}, priority={}, offset={:?}",
            fiber_id,
            priority,
            absolute_offset
        );
        ...
        log::debug!(
            "[DEFERRED] Deferred draw registered, active count={}",
            self.window.fiber.active_deferred_draws.members.len()
        );
    }
...
pub(crate) struct HitboxData {
    ...
    pub(crate) reference_origin: Option<Point<Pixels>>,
    pub(crate) local_bounds: Bounds<Pixels>,
    pub(crate) local_content_mask_bounds: Bounds<Pixels>,
}
...
impl FiberRuntimeHandle<'_> {
    ...
    pub fn insert_hitbox_with_fiber(
        ...
        let reference_origin = self
            ...
            .get(fiber_id.into())
            .map(|bounds| bounds.origin);
        let local_bounds = if let Some(origin) = reference_origin {
            Bounds {
        ...
        let local_content_mask_bounds = if let Some(origin) = reference_origin {
            Bounds {
                origin: content_mask.bounds.origin - origin,
                    ...
                    && self
                        ...
                        .any(|deferred_id| {
                            ...
                            let hitbox = self
                                ...
                                .get(&hitbox_id)
                                .cloned()
                                .or_else(|| self.window.resolve_hitbox_for_event(deferred_id));
                            let Some(hitbox) = hitbox else {
                                return false;
                            };
                            let bounds = hitbox.bounds.intersect(&hitbox.content_mask.bounds);
                            bounds.contains(&mouse_move.position)
                        });
