...
Rendering proceeds in four phases, each with its own caching/skip logic. Change detection happens during **reconcile** via descriptor diffs (hashes + identity/structure changes).
...
0. **Reconcile** — build ephemeral descriptors (elements), diff them against the retained tree, update retained nodes, and compute dirty flags.
   - **Change detection inputs:** each descriptor provides three hashes:
     - **Content hash** — what it displays
     - **Layout hash** — sizing and positioning
     - **Paint hash** — visual properties
   - A mismatch (or a key/structure change) marks a fiber dirty and schedules the appropriate work. Unchanged siblings and cousins are skipped entirely.
1. **Layout** — size and position computation using Taffy, driven by per-fiber `taffy_style` and leaf measurement via retained nodes.
...
- update `taffy_style` / leaf measurement configuration
...
- **Elements become descriptors**: ephemeral values produced by `render()` that describe identity (`VKey`), hashes, and children.
...
For third-party / opaque elements that don’t provide a retained node, GPUI falls back to an “opaque element” escape hatch (always-dirty, correctness-first) while opting out of incremental caching.

### Custom Elements

Custom or opaque elements that don’t participate in descriptor-based change detection always re-render. This preserves correctness while allowing built-in elements to realize the full `O(changed)` benefits.
...
* Reference-equality fast paths before hashing
