...
fn main() {
    ...
    let rngpui_native = rngpui_dir.join("native").join("cpp").join("src");

    // Third-party dependencies (Folly, Boost, etc.) used by React Native bridging
    let third_party = rngpui_dir.join("native").join("third-party");
    ...
    cc::Build::new()
        ...
        .include(third_party.join("glog").join("src"))
