...
pub mod bridging {
    ...
    extern "Rust" {
        ...
        #[cxx_name = "createZedThreads"]
        fn create_zed_threads(id: usize, data_path: &str) -> Box<ZedThreads>;

        #[cxx_name = "addListener"]
        fn zed_threads_add_listener(it_: &mut ZedThreads, event_name: &str) -> Result<()>;

        #[cxx_name = "deleteThread"]
        fn zed_threads_delete_thread(it_: &mut ZedThreads, thread_id: &str) -> Result<bool>;

        #[cxx_name = "listThreads"]
        fn zed_threads_list_threads(it_: &mut ZedThreads) -> Result<String>;

        #[cxx_name = "loadThread"]
        fn zed_threads_load_thread(it_: &mut ZedThreads, thread_id: &str) -> Result<String>;

        #[cxx_name = "removeListeners"]
        fn zed_threads_remove_listeners(it_: &mut ZedThreads, count: f64) -> Result<()>;

        #[cxx_name = "saveThread"]
        fn zed_threads_save_thread(it_: &mut ZedThreads, thread_json: &str) -> Result<bool>;

        #[cxx_name = "searchThreads"]
        fn zed_threads_search_threads(it_: &mut ZedThreads, query: &str) -> Result<String>;
    ...
    extern "Rust" {
        ...
        type ZedThreadsSignal;
        fn get_threads_changed_payload(s: &ZedThreadsSignal) -> ThreadsChangedEvent;
        unsafe fn drop_zed_threads_signal(signal: *mut ZedThreadsSignal);
...
fn create_zed_threads(id: usize, data_path: &str) -> Box<ZedThreads> {
    let ctx = Context::new(id, data_path);
    Box::new(ZedThreads::new(ctx))
}

fn zed_threads_add_listener(it_: &mut ZedThreads, event_name: &str) -> Result<(), anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.add_listener(event_name);
        ret
    })
...
fn zed_threads_delete_thread(it_: &mut ZedThreads, thread_id: &str) -> Result<bool, anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.delete_thread(thread_id);
        ret
    })
}

fn zed_threads_list_threads(it_: &mut ZedThreads) -> Result<String, anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.list_threads();
        ret
    })
}

fn zed_threads_load_thread(it_: &mut ZedThreads, thread_id: &str) -> Result<String, anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.load_thread(thread_id);
        ret
    })
}

fn zed_threads_remove_listeners(it_: &mut ZedThreads, count: f64) -> Result<(), anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.remove_listeners(count);
        ret
    })
...
fn zed_threads_save_thread(it_: &mut ZedThreads, thread_json: &str) -> Result<bool, anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.save_thread(thread_json);
        ret
    })
}

fn zed_threads_search_threads(it_: &mut ZedThreads, query: &str) -> Result<String, anyhow::Error> {
    craby::catch_panic!({
        let ret = it_.search_threads(query);
        ret
    })
}
...
fn get_threads_changed_payload(s: &ZedThreadsSignal) -> ThreadsChangedEvent {
    match s {
        ZedThreadsSignal::ThreadsChanged(payload) => (*payload).clone(),
        _ => panic!("Invalid signal type for get_threads_changed_payload"),
    }
}
