...
pub trait ZedThreadsSpec {
    fn new(ctx: Context) -> Self;
    fn id(&self) -> usize;
    fn emit(&self, signal_name: ZedThreadsSignal) {
        let manager = crate::ffi::bridging::get_signal_manager();
        match signal_name {
            ZedThreadsSignal::ThreadsChanged(data) => {
                let signal = Box::new(ZedThreadsSignal::ThreadsChanged(data));
                let signal_ptr = Box::into_raw(signal) as usize;
                unsafe {
                    manager.emit(self.id(), "threadsChanged", signal_ptr);
                }
            }
        }
    }
...
pub enum ZedThreadsSignal {
    ThreadsChanged(ThreadsChangedEvent),
}
...
impl Default for ThreadsChangedEvent {
    fn default() -> Self {
        ThreadsChangedEvent {
            threads: Vec::default()
        }
